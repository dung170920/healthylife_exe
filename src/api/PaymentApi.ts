...
export const sendRequestToNganLuong = (params: any) => {
  let secureCode = CryptoJS.MD5(
    `${process.env.REACT_APP_MERCHANT_CODE} https://helife.netlify.app/users/payment/success ${process.env.REACT_APP_RECEIVER_EMAIL} nap tien ${params.orderCode} ${params.price} vnd 1 0 0 0 0    ${process.env.REACT_APP_MERCHANT_PASSWORD}`
  );

  const nganluongParams = {
    merchant_site_code: process.env.REACT_APP_MERCHANT_CODE,
    return_url: `https://helife.netlify.app/users/payment/success`,
  ...
  return `https://www.nganluong.vn/checkout.php?${queryString.stringify(
    nganluongParams
  )}`;
};
